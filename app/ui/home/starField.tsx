...
// Star类构造函数外部声明
const Star = function (this: Star, x: number, y: number, radius: number, velocity: { x: number; y: number }) {
    this.x = x;
    this.y = y;
    this.radius = radius;
...
} as any as { new (x: number, y: number, radius: number, velocity: { x: number; y: number }): Star };
...
const StarField: React.FC = () => {
    ...
    return (
        <canvas id="starfield" className="fixed top-0 left-0 w-full h-full  pointer-events-none z-[0]" />
    );
};
